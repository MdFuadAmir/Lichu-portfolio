...
const Header = () => {
    ...
    return (
        <nav className="mx-auto px-4 font-mono shadow-md py-2">
            <div className="flex justify-between items-center">
                ...
                <div className="hidden md:flex gap-5 font-semibold">
                    {
                        links.map((link) => (<Link to={link.path} key={link.id} className="hover:border-green-600 hover:border-b-2 hover:text-green-500">{link.name}</Link>))
                    }
                </div>
                {/* menu optation */}
